(function() {
  ...
  // Array.prototype.slice.call(arguments) 能将具有length属性的对象转成数组,就是将 类数组转化为数组
  eventEmitter.prototype.emit = function(eventName) {
    const argsList = Array.prototype.slice.call(arguments, 1);
    let length = this.sub[eventName].length;
    ...
    proto.append = function() {
      ...
      while (count < 100) {
        if (
          ...
          perList.push(
            imgList[currentPosition - count].offsetHeight + imgList[currentPosition - count].offsetTop
          )
          tmpArr.push(count)
